fetch("https://weatherapp-backend-matteo-pisellis-projects.vercel.app/weather")
  ...
  for (let i = 0; i < document.querySelectorAll(".deleteCity").length; i++) {
    document
      ...
      [i].addEventListener("click", function () {
        fetch(
          `https://weatherapp-backend-matteo-pisellis-projects.vercel.app/weather/${this.id}`,
          { method: "DELETE" }
        )
          .then((response) => response.json())
          .then((data) => {
...
document.querySelector("#addCity").addEventListener("click", function () {
  ...
  fetch(
    "https://weatherapp-backend-matteo-pisellis-projects.vercel.app/weather",
    {
      method: "POST",
